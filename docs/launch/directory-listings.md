...
Chrome Bridge also includes real-profile workflows that are awkward in empty-profile automation. `browser_session_status` gives a redacted auth probe with cookie names/counts and logged-in status, never values. `browser_wait_for_handoff` pauses the agent, focuses the tab, shows a banner, lets a human complete login/2FA/captcha/payment, and resumes when the expected page state appears. Cooperative leasing helps multiple local agents share one real profile without colliding.
